...
import numpy as np
import torch
from general_ocr import Config
from general_ocr.image import tensor2imgs
from general_ocr.parallel import MMDataParallel
    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            if np.prod(img_tensor.shape) == 0:
                imgs = [general_ocr.imread(m['filename']) for m in img_metas]
            else:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='general_ocr visualize for kie model.')
