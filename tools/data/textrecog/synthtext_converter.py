...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        'prepration for general_ocr\'s use')
    parser.add_argument(
...
def load_gt_data(filename, n_proc):
    ...
    return general_ocr.track_parallel_progress(
...
def process(data, img_path_prefix, out_dir):
    ...
    input_img = general_ocr.imread(os.path.join(img_path_prefix, img_path))
    ...
    for i, word in enumerate(words):
        ...
        general_ocr.imwrite(cropped_img, output_image_patch_path)
...
def main():
    ...
    general_ocr.track_parallel_progress(process_with_outdir, data, nproc=args.n_proc)
    print('Done')
