...
class Shear:
    ...
    def _shear_img(self,
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_sheared = general_ocr.imshear(
                img,
                magnitude,
                direction,
    ...
    def _shear_seg(self,
        ...
        for key in results.get('seg_fields', []):
            seg = results[key]
            results[key] = general_ocr.imshear(
                seg,
                magnitude,
                direction,
                border_value=fill_val,
...
class Rotate:
    ...
    Args:
        ...
        scale (int | float): Isotropic scale factor. Same in
            ``general_ocr.imrotate``.
        center (int | float | tuple[float]): Center point (w, h) of the
            rotation in the source image. If None, the center of the
            image will be used. Same in ``general_ocr.imrotate``.
    ...
    def _rotate_img(self, results, angle, center=None, scale=1.0):
        ...
        Args:
            ...
            angle (float): Rotation angle in degrees, positive values
                mean clockwise rotation. Same in ``general_ocr.imrotate``.
            center (tuple[float], optional): Center point (w, h) of the
                rotation. Same in ``general_ocr.imrotate``.
            scale (int | float): Isotropic scale factor. Same in
                ``general_ocr.imrotate``.
        """
        for key in results.get('img_fields', ['img']):
            img = results[key].copy()
            img_rotated = general_ocr.imrotate(
    ...
    def _rotate_seg(self,
        ...
        for key in results.get('seg_fields', []):
            ...
            results[key] = general_ocr.imrotate(
...
class Translate:
    ...
    def _translate_img(self, results, offset, direction='horizontal'):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key].copy()
            results[key] = general_ocr.imtranslate(
    ...
    def _translate_seg(self,
        ...
        for key in results.get('seg_fields', []):
            seg = results[key].copy()
            results[key] = general_ocr.imtranslate(seg, offset, direction,
...
class ColorTransform:
    ...
    def _adjust_color_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            img = results[key]
            results[key] = general_ocr.adjust_color(img, factor).astype(img.dtype)
...
class EqualizeTransform:
    ...
    def _imequalize(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = general_ocr.imequalize(img).astype(img.dtype)
...
class BrightnessTransform:
    ...
    def _adjust_brightness_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = general_ocr.adjust_brightness(img,
...
class ContrastTransform:
    ...
    def _adjust_contrast_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = general_ocr.adjust_contrast(img, factor).astype(img.dtype)
