...
class LoadAnnotations:
    ...
    Args:
        ...
        file_client_args (dict): Arguments to instantiate a FileClient.
            See :class:`general_ocr.fileio.FileClient` for details.
    ...
    def _load_semantic_seg(self, results):
        ...
        if self.file_client is None:
            self.file_client = general_ocr.FileClient(**self.file_client_args)

        filename = osp.join(results['seg_prefix'],
                            results['ann_info']['seg_map'])
        img_bytes = self.file_client.get(filename)
        results['gt_semantic_seg'] = general_ocr.imfrombytes(
...
class LoadPanopticAnnotations(LoadAnnotations):
    ...
    def _load_masks_and_semantic_segs(self, results):
        ...
        img_bytes = self.file_client.get(filename)
        pan_png = general_ocr.imfrombytes(
...
class LoadImageFromFile:
    ...
    Args:
        to_float32 (bool): Whether to convert the loaded image to a float32
            ...
            Defaults to False.
        color_type (str): The flag argument for :func:`general_ocr.imfrombytes`.
            Defaults to 'color'.
        file_client_args (dict): Arguments to instantiate a FileClient.
            See :class:`general_ocr.fileio.FileClient` for details.
            Defaults to ``dict(backend='disk')``.
    ...
    def __call__(self, results):
        ...
        img_bytes = self.file_client.get(filename)
        img = general_ocr.imfrombytes(img_bytes, flag=self.color_type)
        if self.to_float32:
...
class LoadMultiChannelImageFromFiles:
    ...
    def __call__(self, results):
        ...
        for name in filename:
            ...
            img.append(general_ocr.imfrombytes(img_bytes, flag=self.color_type))
...
class LoadImageFromNdarray(LoadImageFromFile):
    ...
    def __call__(self, results):
        ...
        if self.color_type == 'grayscale' and img.shape[2] == 3:
            img = general_ocr.bgr2gray(img, keepdim=True)
        if self.color_type == 'color' and img.shape[2] == 1:
            img = general_ocr.gray2bgr(img)
        if self.to_float32:
            img = img.astype(np.float32)
