...
class ONNXRuntimeDetector(TextDetectorMixin, SingleStageTextDetector):
    ...
    def __init__(self,
        ...
        ort_custom_op_path = ''
        try:
            from general_ocr.ops import get_onnxruntime_op_path
            ort_custom_op_path = get_onnxruntime_op_path()
        except (ImportError, ModuleNotFoundError):
            warnings.warn('If input model has custom op from general_ocr, \
                you may have to build general_ocr with ONNXRuntime from source.')
...
class TensorRTDetector(TextDetectorMixin, SingleStageTextDetector):
    ...
    def __init__(self,
        ...
        SingleStageTextDetector.__init__(self, cfg.model.backbone,
                                         ...
                                         cfg.model.pretrained)
        TextDetectorMixin.__init__(self, show_score)
        from general_ocr.tensorrt import TRTWrapper, load_tensorrt_plugin
        try:
            load_tensorrt_plugin()
        except (ImportError, ModuleNotFoundError):
            warnings.warn('If input model has custom op from general_ocr, \
                you may have to build general_ocr with TensorRT from source.')
        model = TRTWrapper(
