...
import general_ocr.utils as utils
...
def show_pred_gt(preds,
    ...
    pred_color = general_ocr.color_val('red')
    gt_color = general_ocr.color_val('blue')
    thickness = 1
    ...
    for gt in gts:
        cv2.polylines(
            ...
            thickness=thickness)
    if show:
        general_ocr.imshow(img, win_name, wait_time)
    if out_file is not None:
        general_ocr.imwrite(img, out_file)
...
def imshow_pred_boundary(img,
    ...
    boundary_color = general_ocr.color_val(boundary_color)
    text_color = general_ocr.color_val(text_color)
    font_scale = 0.5
...
def imshow_text_char_boundary(img,
    ...
    img = general_ocr.imread(img)
    char_color = [general_ocr.color_val('blue'), general_ocr.color_val('green')]
    text_color = general_ocr.color_val('red')
    text_inx = 0
...
def imshow_edge_node(img,
    ...
    if show:
        general_ocr.imshow(vis_img, win_name, wait_time)
    if out_file is not None:
        general_ocr.imwrite(vis_img, out_file)
...
def det_recog_show_result(img, end2end_res, out_file=None):
    ...
    if out_file:
        general_ocr.imwrite(out_img, out_file)
