...
def generate_inputs_and_wrap_model(config_path,
    ...
    try:
        from general_ocr.onnx.symbolic import register_extra_symbolics
    except ModuleNotFoundError:
        raise NotImplementedError('please update general_ocr to version>=v1.0.4')
...
def build_model_from_cfg(config_path, checkpoint_path, cfg_options=None):
    ...
    cfg = general_ocr.Config.fromfile(config_path)
...
def preprocess_example_input(input_config):
    ...
    one_img = general_ocr.imread(input_path)
    one_img = general_ocr.imresize(one_img, input_shape[2:][::-1])
    ...
    if 'normalize_cfg' in input_config.keys():
        ...
        one_img = general_ocr.imnormalize(one_img, mean, std, to_rgb=to_rgb)
