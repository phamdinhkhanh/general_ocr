...
def _get_mmcv_home():
    mmcv_home = os.path.expanduser(
        os.getenv(
            ENV_MMCV_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'general_ocr')))
...
def get_external_models():
    ...
    default_json_path = osp.join(general_ocr.__path__[0], 'model_zoo/open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(general_ocr.__path__[0], 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(general_ocr.__path__[0],
...
class CheckpointLoader:
    ...
    def load_checkpoint(cls, filename, map_location=None, logger=None):
        ...
        general_ocr.print_log(f'Use {class_name} loader', logger)
...
def save_checkpoint(model, filename, optimizer=None, meta=None):
    ...
    meta.update(mmcv_version=general_ocr.__version__, time=time.asctime())
    ...
    if filename.startswith('pavi://'):
        ...
        general_ocr.mkdir_or_exist(osp.dirname(filename))
