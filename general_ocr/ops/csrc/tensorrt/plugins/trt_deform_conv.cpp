...
static const char *PLUGIN_NAME{"general_ocrDeformConv2d"};
}  // namespace
...
size_t DeformableConvPluginDynamic::getWorkspaceSize(
  ...
  int sizeof_dtype = general_ocr::getElementSize(outputs[0].type);
  ...
  size_t col_size =
      general_ocr::getAlignedSize(nInputPlane * kW * kH * im2col_step * outputHeight *
                           outputWidth * sizeof_dtype);

  size_t out_size = 0;
  if (im2col_step != 1)
    out_size = general_ocr::getAlignedSize(batch_size * nOutputPlane * outputHeight *
                                    outputWidth * sizeof_dtype);
