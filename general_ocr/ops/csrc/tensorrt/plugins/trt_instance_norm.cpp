...
constexpr const char* PLUGIN_NAME{"general_ocrInstanceNormalization"};
}  // namespace
...
size_t InstanceNormalizationDynamic::getWorkspaceSize(
  ...
  int elem_size = general_ocr::getElementSize(inputs[1].type);
  return general_ocr::getAlignedSize(n * c * elem_size) * 2;
}
...
int InstanceNormalizationDynamic::enqueue(
  ...
  int elem_size = general_ocr::getElementSize(inputDesc[1].type);

  void* n_scales = (void*)workspace;
  void* n_bias = (void*)(workspace + general_ocr::getAlignedSize(n * c * elem_size));
