...
from general_ocr.utils import deprecated_api_warning
...
class NMSop(torch.autograd.Function):
    ...
    def symbolic(g, bboxes, scores, iou_threshold, offset, score_threshold,
        ...
        is_trt_backend = os.environ.get('ONNX_BACKEND') == 'general_ocrTensorRT'
        if has_custom_op and (not is_trt_backend):
            return g.op(
                'general_ocr::NonMaxSuppression',
                bboxes,
                scores,
...
class SoftNMSop(torch.autograd.Function):
    ...
    def symbolic(g, boxes, scores, iou_threshold, sigma, min_score, method,
        ...
        nms_out = g.op(
            'general_ocr::SoftNonMaxSuppression',
            boxes,
            scores,
            iou_threshold_f=float(iou_threshold),
            sigma_f=float(sigma),
