...
import numpy as np
import torch
from general_ocr.image.misc import tensor2imgs
from general_ocr.runner import load_checkpoint
from general_ocr.utils.config import Config
from general_ocr.apis import init_detector
from general_ocr.apis.inference import model_inference
from general_ocr.core.visualize import det_recog_show_result
from general_ocr.datasets.kie_dataset import KIEDataset
from general_ocr.datasets.pipelines.crop import crop_img
from general_ocr.models import build_detector
from general_ocr.utils.box_util import stitch_boxes_into_lines
from general_ocr import list_from_file
from general_ocr.utils.model import revert_sync_batchnorm
from general_ocr import imshow
...
class GENERAL_OCR:
    ...
    def readtext(self,
        ...
        # Send args and models to the GENERAL_OCR model inference API
        ...
        for arr, output, export, det_recog_result in zip(
            ...
            if output or args.imshow:
                ...
                if args.imshow and not self.kie_model:
                    general_ocr.imshow(res_img, 'inference results')
            if not args.details:
                simple_res = {}
            ...
            if export:
                general_ocr.dump(final_result, export, indent=4)
    ...
    def single_pp(self, result, model):
        for arr, output, export, res in zip(self.args.arrays, self.args.output,
            ...
            if export:
                general_ocr.dump(res, export, indent=4)
    ...
    def _args_processing(self, args):
        ...
        args.arrays = [general_ocr.imread(x) for x in img_list]
